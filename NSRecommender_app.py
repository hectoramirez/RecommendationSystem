...
if btn:
    ...
    st.markdown('__The full code of this project can be found in '
                '[this repository](https://github.com/hectoramirez/RecommendationSystem).__')
...
for section in soup.find_all('h2'):
    ...
    st.markdown('Using [NLTK](https://www.nltk.org) for natural language processing, I defined '
                'the rules for tokenizing and stemming the plots. Tokenizing is the process by '
                'which we break down articles into individual sentences or words; whereas '
                'Stemming is the process by which we bring down a word from its different forms '
                'to the root word. The latter helps us establish meaning to different forms of the same '
                'words without having to deal with each form separately.')
    st.code(""" 
